...
void instanceHit(rayQueryEXT ray_query, TraversalPayload load, SceneNode node, out float tNear, out float tFar) {
	...
	mat4 world_to_object = directChild.world_to_object;
	vec3 origin = (world_to_object * vec4(load.transformed_origin, 1)).xyz;
	vec3 direction = (world_to_object * vec4(load.transformed_direction, 0)).xyz;
	vec3 x = (world_to_object * vec4(load.transformed_x, 0)).xyz;


	SceneNode next = nodes[childIndices[directChild.childrenIndex + pIdx]];
	...
	nextLoad.world_to_object = next.world_to_object * directChild.world_to_object * load.world_to_object;
...
bool ray_trace_loop(vec3 rayOrigin, vec3 rayDirection, float t_max, uint root, out vec3 tuv, out int triangle_index, out TraversalPayload resultPayload) {
	...
	while (stackSize > 0) {
		...
		recordQuery(node.Index, node.level, rayOrigin, rayOrigin + best_t * rayDirection, triangleIntersections, instanceIntersections);
	}
